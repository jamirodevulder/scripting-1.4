        ...
        img.onload = function(){
            ...
            for (let i = 0; i < textData.data.length; i += 4) {
                if (textData.data[i+3] !== 0) { //zwart in geheime tekst
                    if (imgData.data[i+2] % 10 == 8) {
                        //doe niets
                    }
                    else if (imgData.data[i+2] > 248) {
                        imgData.data[i+2] = 248; //begrens de waarde
                    }
                    else {
                        while (imgData.data[i+2] % 10 != 8) {
                            imgData.data[i+2]++;//zorg dat op 8 eindigt
                        }
                    }
                    pixelsInMsg++;
                }
